...
GetSimple (){
local data=$(grep "$1" "$2" | tail -n 1 | awk -v p="$3" '{print $p}')
echo "$data"
}

energia=$(GetSimple "SCF Done" $archivo 5)
temperatura=$(GetSimple "Temperature" $archivo 2)
presion=$(GetSimple "Pressure" $archivo 5)
zpe=$(GetSimple "Zero-point correction" $archivo 3)
u=$(GetSimple "Thermal correction to Energy" $archivo 5)
entalpia_h=$(GetSimple "Thermal correction to Enthalpy" $archivo 5)
gibbs=$(GetSimple "Thermal correction to Gibbs Free Energy" $archivo 7)
